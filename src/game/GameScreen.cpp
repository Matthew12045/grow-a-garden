...
void GameScreen::drawPlant(Plant* p, sf::Vector2f s) {
    ...
    if (ripe) {
        // Find seed def for this crop name
        std::string seedKey = "Carrot Seed";
        for (const auto& d : catalogue_)
            if (d.cropName == p->getName()) { seedKey = d.name; break; }
        const auto* def = findItem(seedKey);
        if (def) drawCropIcon(*def, {cx, cy}, 70.f);
    } else {
        // Growing: stem + leaves scaled by ratio
        // Use crop-specific colours
        sf::Color stemCol = Pal::SPROUT;
        for (const auto& d : catalogue_)
            if (d.cropName == p->getName()) { stemCol = d.iconAccent; break; }

        float h = 10.f + ratio * (CELL_SZ * 0.38f);
...
// ─────────────────────────────────────────────────────────────────────
//  Crop icon — used in shop cards AND in ripe cells
// ─────────────────────────────────────────────────────────────────────
void GameScreen::drawCropIcon(const ShopItemDef& def, sf::Vector2f c, float sz) {
    ...
    if (n == "Carrot") {
        ...
        window_.draw(pod);
        for (int i = -1; i <= 1; ++i)
            rect(i*pw*0.4f, 0, sz*0.03f, ph*1.6f, {(uint8_t)(col.r+20),(uint8_t)(col.g+15),(uint8_t)(col.b+10)});
...
void GameScreen::drawSeedCard(const ShopItemDef& def, sf::FloatRect b, sf::Vector2f mouse) {
    ...
    drawCropIcon(def, {iconX, iconY}, iconSz * 0.78f);
...
void GameScreen::drawToolCard(const ShopItemDef& def, sf::FloatRect b, sf::Vector2f mouse) {
    ...
    iconBg.setFillColor({30,18,6});
    window_.draw(iconBg);
    drawCropIcon(def, {iconX, iconY}, iconSz*0.72f);
